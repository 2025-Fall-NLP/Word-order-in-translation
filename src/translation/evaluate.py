...
class BLEUMetric(BaseEvalMetric):
    ...
    def compute(self, hypotheses: List[str], references: List[str], sources: Optional[List[str]] = None) -> float:
        return self._bleu.compute(
            predictions=hypotheses, references=[[r] for r in references], tokenize=self.tokenize
        )["score"]
...
class COMETMetric(BaseEvalMetric):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        model_path = download_model(config.get("model", "Unbabel/wmt20-comet-da"))
    ...
    def compute(self, hypotheses: List[str], references: List[str], sources: Optional[List[str]] = None) -> float:
        ...
        return self._model.predict(data, batch_size=self.batch_size, gpus=self._gpus, num_workers=0, progress_bar=False).system_score
